...
sim_plan_ss_2d = function(xlim, ylim, n_sp, radius, by){
  ...
  if(by == "row"){
    ...
    y_sp_raw = runif(n = n_sp, min = ylim[1], max = y_lim[2])
...
sim_plan_2d = function(method_sp, n_sp, xlim, ylim, radius, n_strata, by){
  if(method_sp == "srs"){
    sim_plan_srs_2d(n_sp = n_sp, xlim = x_lim, ylim = y_lim, radius = radius)
  } else if (method_sp == "strs"){
    sim_plan_strs_2d(n_sp = n_sp, n_strata = n_strata, by = by, xlim = x_lim, ylim = y_lim, radius = radius)
  } else if (method_sp == "ss"){
    sim_plan_ss_2d(xlim = x_lim, ylim = y_lim, n_sp = n_sp, radius = radius, by = by)
  } else {
