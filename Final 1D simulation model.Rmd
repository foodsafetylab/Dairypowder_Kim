...
**Sampling contamination**
  ...
  * `LOC` = the limit of contribution of contamination. By default, it is set at 0.001 (Both `spread_radius` and `LOC` determine the shape of decay function that describes how much contamination from the source is contributed to a target point). For 1D simulation use 1. 
...
**Sampling strategies**
  ...
  * `n_strata` = the number of strata (applicable to *1D Stratified random sampling*)
  * `by` = the side along which the field is divided into strata. It is either "row" or "column" (applicable to *1D Stratified random sampling*) **OR** the side along which a sample is taken every k steps (applicable to *1D Systematic sampling*).

**Sample Assay**
  
  * `m_sp` = individual sample mass (g)
  * `method_det` = method of detection  
      + Plating: LOD = 10 CFU/g  
...
### 6. Save simulation result in the folder if you want 
```{r,eval=FALSE}
write.csv(data_cleaned0,"C:\\Users\\YOUR folder PATH")
